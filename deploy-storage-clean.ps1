...
if (-not $storageExists) {
    Write-Error "❌ Storage Account '$StorageAccountName' nao encontrada no Resource Group '$ResourceGroupName'"
    exit 1
}
...
if (-not (Test-Path "$SourcePath/index.html")) {
    Write-Error "❌ Arquivo index.html nao encontrado em '$SourcePath'"
    exit 1
}
...
Write-Host "🖼️  Imagens disponiveis em: https://www.sistemasave.com.br/img/[nome-da-imagem]" -ForegroundColor Cyan
